    ...
    if (projectDataStr) {
        try {
            ...
            if (imageEl) {
                if (projectData.image) {
                    ...
                    imageEl.onerror = function() {
                        console.error('Failed to load project image:', projectData.image);
            ...
            if (additionalContentEl && projectData.additionalContent && Array.isArray(projectData.additionalContent)) {
                ...
                while (i < projectData.additionalContent.length) {
                    const section = projectData.additionalContent[i];
                    const delayClass = sectionIndex < 4 ? `animate-fade-in-delay-${sectionIndex + 4}` : 'animate-fade-in-delay-7';
                    
                    // Check if we have consecutive images
                    if (section.type === 'image' && section.src) {
                        const imageGroup = [];
                        let j = i;
                        
                        // Collect consecutive images
                        while (j < projectData.additionalContent.length && 
                               projectData.additionalContent[j].type === 'image' && 
                               projectData.additionalContent[j].src) {
                            imageGroup.push(projectData.additionalContent[j]);
                            j++;
                        }
                        
                        // Check if this is EmpowerPWD project (web screenshots need different handling)
                        const isEmpowerPWD = projectData.title && projectData.title.toLowerCase().includes('empowerpwd');
                        const isSoundSprint = projectData.title && projectData.title.toLowerCase().includes('soundsprint');
                        
                        // If we have 2+ images, group them in a grid
                        if (imageGroup.length >= 2) {
                            const gridDiv = document.createElement('div');
                            // For EmpowerPWD, use single column full-width layout
                            // For SoundSprint, use 2-column grid for mobile screenshots
                            // For others, use default 2-column grid
                            if (isEmpowerPWD) {
                                gridDiv.className = `project-additional-section project-images-grid ${delayClass}`;
                                gridDiv.setAttribute('data-project', 'empowerpwd');
                            } else if (isSoundSprint) {
                                gridDiv.className = `project-additional-section project-images-grid ${delayClass}`;
                                gridDiv.setAttribute('data-project', 'soundsprint');
                            } else {
                                gridDiv.className = `project-additional-section project-images-grid ${delayClass}`;
                            }
                            
                            imageGroup.forEach((imgSection, imgIndex) => {
                                const img = document.createElement('img');
                                img.src = imgSection.src;
                                img.alt = imgSection.alt || `Project detail ${sectionIndex + imgIndex + 1}`;
                                img.className = 'project-additional-image w-full h-auto rounded-lg';
                                img.loading = 'lazy';
                                img.onerror = function() {
                                    console.error('Failed to load image:', imgSection.src);
                                    this.style.display = 'none';
                                };
                                gridDiv.appendChild(img);
                            });
                            
                            additionalContentEl.appendChild(gridDiv);
                        ...
                        const img = document.createElement('img');
                        img.src = section.src;
                            img.alt = section.alt || `Project detail ${sectionIndex + 1}`;
                        img.className = 'project-additional-image w-full h-auto rounded-lg';
                            img.loading = 'lazy';
                            img.onerror = function() {
                                console.error('Failed to load image:', section.src);
                                this.style.display = 'none';
                            };
